...
/**
 * @brief Desvincula un fichero
 *
 * @param path directorio del fichero
 * @return 0 en exito y <0 en error
 **/
static int my_unlink (const char * path){
    int idxNodoI, idxDir, i, nBlock;

    fprintf(stderr, "--->>>my_unlink: path %s\n", path);
    //Busca si existe el archivo en el path
    if ((idxDir = findFileByName(&myFileSystem, (char*) path + 1)) == -1 ) {
        fprintf(stderr,"Error intentando encontrar el fichero en in my_read\n");
        return -ENOENT;
    }
    myFileSystem.directory.files[idxDir].freeFile = true;       // Indica que el archivo esta libre
    myFileSystem.directory.numFiles--;

    /*if ((*/idxNodoI = myFileSystem.directory.files[idxDir].nodeIdx; //) == NULL) return -ENOENT;
    myFileSystem.nodes[idxNodoI]->freeNode = true;      // Indica que el nodo esta libre
    myFileSystem.numFreeNodes++;

    //Libera los bloques en el BitMap y actualizamos el superBloque
    for(i = 0; i < myFileSystem.nodes[idxNodoI]->numBlocks; i++){
        nBlock = myFileSystem.nodes[idxNodoI]->blocks[i];
        myFileSystem.bitMap[nBlock] = 0;
    ...
    free(myFileSystem.nodes[idxNodoI]);     // Libera la memoria reservada para la estructura del nodo
...
/**
 * @brief Lee datos de un fichero abierto
 *
 * @param path directorio del fichero
 * @param buf buffer donde escribiremos los datos leidos
 * @param size cantidad de datos a leer
 * @param offset offset del fichero a empezar a leer
 * @param fi estructura FUSE vinculada al fichero
 * @return 0 en exito y <0 en error
 **/
...
static int my_read(const char* path, char* buf, size_t size, off_t offset, struct fuse_file_info* fi){
    ...
    //Comprueba si el offset se pasa del tamaño del archivo
    if (offset > node->fileSize) {
        fprintf(stderr,"Error en el offset de my_read\n");
        return -EINVAL;
    }
    // Comprueba si el size es mayor a lo que queda por leer en el archivo
    if ((offset + size) >= node->fileSize) size = node->fileSize - offset;
    bytes2Read = size;
    // Lee los datos
    while(bytes2Read) {
        int i;
        ...
        if( readBlock(&myFileSystem, currentBlock, &buffer)==-1 ) {
            fprintf(stderr,"Error leyendo bloques en my_read\n");
            return -EIO;
        }
...
struct fuse_operations myFS_operations = {
    ...
    .unlink     = my_unlink,                    // Desvincula un fichero
    .read       = my_read,                      // Lee un fichero
}; 
